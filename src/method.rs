...
pub enum ControlMethod {
    ...
    /// node advertises today. Shared by the manual "reset mirrors" action and the same primitive
    /// the node's DAILY detector runs (dig-node#570) -- ONE reconcile primitive, exposed two ways.
    /// See [`crate::results::MirrorReconcileResult`] for the three outcomes a caller must be able
    /// to tell apart, and dig_ecosystem#3203 for why `refused` must mean NOTHING was spent.
    MirrorReconcile,
