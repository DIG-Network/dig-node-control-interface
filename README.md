...
| `control.wallet.reservations.held` | T | del | — | `{reserved:[{coin_id, reservation_id, expires_at_unix}], as_of_unix}`; the coins committed to in-flight spends. `reserved:[]` means NOTHING is held; a set that cannot be read is `-32045`, never an empty list. Narrows SELECTION, never a balance |
| `control.wallet.reservations.reserve` | T | del | `{coin_ids:[string], ttl_secs?:u64}` | `{reservation_id, coin_ids, expires_at_unix, ttl_secs}`; ALL the named coins or none. A clash is `-32044 WALLET_COINS_RESERVED` — a WAIT, never a shortfall — and reserves nothing. The returned `ttl_secs` is the lifetime the node APPLIED, which may be shorter than requested |
