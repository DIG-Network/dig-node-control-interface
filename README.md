...
| `control.wallet.sends` | T | del | `{after_seq:u64=0, limit?:u32}` | `{sends:[WalletSendRecord], cursor, latest}`; TOKEN-GATED: the answer says when this wallet is SPENDING. `net_outflow` is what LEFT (owned inputs − owned outputs), never a spent coin's amount, and it INCLUDES the fee — chain observation cannot split them. CONFIRMED only; one row per height. Resume from `cursor`, never `latest` |
