...
| `control.wallet.coinsByParent` | — | del | `{parent_coin_id:string}` (64-hex, `0x` accepted) | `{coins:[WalletCoinRecord], source, synced, peak_height}`; the parent's DIRECT children, ONE hop, never a recursive walk. `coins:[]` = no known children, never an unreachable chain |
