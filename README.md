...
| `control.mirror.reconcile` | T | own | `{dry_run?:bool=false}` | `{outcome:"planned", capsules, reclaims, creates, estimated_cost_dig_base_units, url_advertised, url_current}` \| `{outcome:"refused", reason, url_current}` \| `{outcome:"completed", reclaimed, created, url}` \| `{outcome:"partial", reclaimed, created, remaining, reason}`; reconciles this node's mirror coins to its CURRENT advertise URL — reclaim every coin advertising a stale one, then recreate it advertising today's. Shared by the manual "reset mirrors" action and dig-node#570's automatic epoch pass. `dry_run:true` prices the plan and spends nothing. `refused` (`address_uncorroborated` \| `url_unchanged` \| `no_mirror_coins` \| `insufficient_funds` \| `reconcile_in_progress` \| `advertise_off`) means NOTHING was spent — the node validates the corroborated URL and checks affordability BEFORE reclaiming anything, and does nothing at all rather than reclaim coins it cannot afford to recreate. `partial` means the affordable PREFIX ran; reclaim-first ordering means that prefix leaves the node no worse off than before the call |
